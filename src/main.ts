...
const isObj = <T>( argNB : T): boolean => {
 ...
 const isTrue = <T>( argB : T): /* // will return an object and state wjether its an object(true) or not (false) */ {
    argB : T, is : boolean} => {
    if (Array.isArray(argB) && !argB.length)/* i.e should not be an empty array) */ 
    {
        return { argB, is: false}
    } 
    if (isObj(argB) && !Object.keys( argB as keyof T).length)
...
const checkBoolValue= <T>( argB : T)/* // will return an object and state wjether its an object(true) or not (false) */ 
   : BoolCheck<T>  => {
    if (Array.isArray(argB) && !argB.length)/* i.e should not be an empty array) */ 
    {
...
const processUser  = <T extends HasId>(user : T) : T =>   /* what we are doing here is narrowing the generic type henece the type must have an id property */
{
...
const getUsersProperty = <
    T extends HasId,
    K extends keyof T //we are building K as keyof the first type we pass in
 >
 (users: T[],key : K ) : T[K][] => 
    {
...
const myState = new StateObject<(string| number | boolean | any)[]> ([10])
